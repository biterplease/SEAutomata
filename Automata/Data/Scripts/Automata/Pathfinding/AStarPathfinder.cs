...
namespace Automata.Pathfinding
{
    public static class AStarPathfinder
    {
        ...
        private static bool FindPath(ref PathfindingContext context, ref Vector3D start, ref Vector3D end, List<Vector3D> pathOutput)
        {
            ...
            // Reset pool cursor and working sets — no allocations from here on
            context.NodePoolIndex = 0;
            ...
            while (context.OpenQueue.Count > 0 && nodesProcessed < context.MaxPathNodes)
            {
                ...
                GetNeighbors(ref context, ref currentPos, gridSpacing, context.NeighborBuffer);
        ...
        private static void GetNeighbors(ref PathfindingContext context, ref Vector3I position, double gridSpacing, List<Vector3I> output)
        {
            output.Clear();

            // Add confirmed-clear neighbors from the persistent adjacency graph.
            // These edges were recorded by PerformRaycast, so they represent directions
            // that have been verified free of obstacles at adjacency-grid resolution.
            if (context.ConfirmedEdges != null)
            {
                Vector3D worldPos = GridToWorld(ref position, gridSpacing);
                Vector3I adjPos = PathfindingContext.WorldToAdjGrid(ref worldPos);
                List<Edge> confirmed = context.ConfirmedEdges.GetNeighbors(adjPos);
                for (int i = 0; i < confirmed.Count; i++)
                {
                    Vector3I adjNeighbor = confirmed[i].Target;
                    Vector3D neighborWorld = PathfindingContext.AdjGridToWorld(ref adjNeighbor);
                    output.Add(WorldToGrid(ref neighborWorld, gridSpacing));
                }
            }

            // Also add all 26 standard directional neighbors, skipping any already present
            // from the adjacency list to avoid duplicate open-set entries.
            int forwardIndex = (int)context.ControllerForwardDirection;
            Vector3I[] directions = DIRECTIONS_BY_FORWARD[forwardIndex];
            for (int i = 0; i < directions.Length; i++)
            {
                Vector3I neighbor = position + directions[i];
                bool duplicate = false;
                for (int j = 0; j < output.Count; j++)
                {
                    if (output[j] == neighbor) { duplicate = true; break; }
                }
                if (!duplicate)
                    output.Add(neighbor);
            }
        ...
        private static bool IsValidPosition(ref PathfindingContext context, ref Vector3D position)
        {
            ...
            if (context.KnownObstacles != null)
            {
                ...
                {
                    if (context.KnownObstacles[i].BoundingBox.Contains(position) != ContainmentType.Disjoint)
                        return false;
                }
            }
        ...
        {
            AStarNode node = context.NodePool[context.NodePoolIndex++];
            node.Reset();
            return node;
        }
