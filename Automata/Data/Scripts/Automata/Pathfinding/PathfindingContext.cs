...
namespace Automata.Pathfinding
{
    ...
    public struct PathfindingContext
    {
        ...
        public AStarNode[] NodePool; // Pre-allocated; NodePoolIndex is the bump-pointer cursor
        ...
        // Confirmed-clear edge graph: populated by PerformRaycast, queried by A*
        public const double AdjacencyGridSpacing = 50.0;
        public AdjacencyList ConfirmedEdges;
        ...
        public static Vector3I WorldToAdjGrid(ref Vector3D worldPos)
        {
            return new Vector3I(
                (int)Math.Round(worldPos.X / AdjacencyGridSpacing),
                (int)Math.Round(worldPos.Y / AdjacencyGridSpacing),
                (int)Math.Round(worldPos.Z / AdjacencyGridSpacing)
            );
        }

        public static Vector3D AdjGridToWorld(ref Vector3I adjPos)
        {
            return new Vector3D(
                adjPos.X * AdjacencyGridSpacing,
                adjPos.Y * AdjacencyGridSpacing,
                adjPos.Z * AdjacencyGridSpacing
            );
        }
    }
}