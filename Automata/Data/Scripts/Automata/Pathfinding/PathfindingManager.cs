...
namespace Automata.Pathfinding
{
    ...
    public class PathfindingManager
    {
        ...
        {
            config = pathfindingConfig ?? AutomataSession.GetConfig()?.Pathfinding;
        ...
        public void Close()
        {
            ...
            if (context.ConfirmedEdges != null)
                context.ConfirmedEdges.Clear();
        ...
        private bool PerformRaycast(ref PathfindingRequest request)
        {
            ...
            // Record confirmed-clear edges so A* can use them on future calls
            if (!hasObstacle)
                AddConfirmedEdges(ref request);

            return true;
        }

        /// <summary>
        /// Walk along a confirmed-clear ray segment and record each adjacency-grid hop as a
        /// bidirectional edge. Called only when PerformRaycast found no obstacle.
        /// </summary>
        private void AddConfirmedEdges(ref PathfindingRequest request)
        {
            Vector3D dir = request.RaycastDirection;
            double dirLen = dir.Length();
            if (dirLen < 0.001) return;
            dir /= dirLen;

            double totalDist = request.RaycastDistance;
            Vector3D pos = request.RaycastStart;
            Vector3I prevAdj = PathfindingContext.WorldToAdjGrid(ref pos);

            for (double stepped = PathfindingContext.AdjacencyGridSpacing; stepped <= totalDist; stepped += PathfindingContext.AdjacencyGridSpacing)
            {
                Vector3D nextPos = request.RaycastStart + dir * stepped;
                Vector3I nextAdj = PathfindingContext.WorldToAdjGrid(ref nextPos);
                if (nextAdj != prevAdj)
                {
                    context.ConfirmedEdges.AddBidirectionalEdge(prevAdj, nextAdj, (float)PathfindingContext.AdjacencyGridSpacing);
                    prevAdj = nextAdj;
                }
            }

            // Ensure the endpoint itself is connected
            Vector3D endPos = request.RaycastStart + dir * totalDist;
            Vector3I endAdj = PathfindingContext.WorldToAdjGrid(ref endPos);
            if (endAdj != prevAdj)
                context.ConfirmedEdges.AddBidirectionalEdge(prevAdj, endAdj, (float)PathfindingContext.AdjacencyGridSpacing);
        }
        ...
        private void InitializeContext()
        {
            ...
            context.KnownObstacles = new List<PathfindingContext.ObstacleData>();
            context.RaycastCache = new HashSet<Vector3D>();
            context.ConfirmedEdges = new AdjacencyList();

            // Pre-allocate A* node pool and fixed-size priority queue once; reused every search
            context.MaxAStarNodes = context.MaxPathNodes;
            context.NodePool = new AStarNode[context.MaxAStarNodes];
            for (int i = 0; i < context.MaxAStarNodes; i++)
                context.NodePool[i] = new AStarNode();
            context.OpenQueue = new FastPriorityQueue<AStarNode>(context.MaxAStarNodes);
            context.OpenSet = new Dictionary<Vector3I, AStarNode>(context.MaxAStarNodes);
            context.ClosedSet = new Dictionary<Vector3I, AStarNode>(context.MaxAStarNodes);
            context.NeighborBuffer = new List<Vector3I>(26);
            context.PathBuffer = new List<Vector3D>(context.MaxAStarNodes);
        ...
        // {
        //     context.ThrustProfile = new ThrustProfile();
