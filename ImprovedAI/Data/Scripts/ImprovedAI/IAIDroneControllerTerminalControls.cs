    ...
    public class IAIDroneControllerTerminalControls
    {
        ...
        public static void CreateControls()
        {
            ...
            // Operation Mode
            {
                ...
                {
                    list.Add(new MyTerminalControlComboBoxItem() { Key = (long)Drone.OperationMode.StandAlone, Value = MyStringId.GetOrCompute("Stand Alone") });
                    list.Add(new MyTerminalControlComboBoxItem() { Key = (long)Drone.OperationMode.ManagedByScheduler, Value = MyStringId.GetOrCompute("Managed by Scheduler") });
                    list.Add(new MyTerminalControlComboBoxItem() { Key = (long)Drone.OperationMode.ManagedByPlayer, Value = MyStringId.GetOrCompute("Managed by Player") });
                };
            ...
            // Managing Scheduler
            {
                var c = MyAPIGateway.TerminalControls.CreateControl<IMyTerminalControlListbox, IMyRemoteControl>(IdPrefix + "List_ManagingScheduler");
                c.Title = MyStringId.GetOrCompute("Managing Scheduler");
                c.Tooltip = MyStringId.GetOrCompute("Select which scheduler manages this drone");
                c.Visible = CustomVisibleCondition;
                c.Enabled = (b) =>
                {
                    var logic = GetBlock(b);
                    return logic != null && logic.Terminal_OperationModeValue == (long)Drone.OperationMode.ManagedByScheduler;
                };
                c.Multiselect = false;
                ...
                c.ListContent = (b, items, selected) =>
                {
                    ...
                    if (logic != null)
                    {
                        logic.Terminal_FillSchedulerList(items, selected);
                    }
                };
                c.ItemSelected = (b, selected) =>
                {
                    ...
                    {
                        logic.Terminal_ManagingScheduler = (long)selected[0].UserData;
                    }
                };
                c.SupportsMultipleBlocks = false;
                MyAPIGateway.TerminalControls.AddControl<IMyRemoteControl>(c);
            ...
            // Enforce Home Orientation
            {
                var c = MyAPIGateway.TerminalControls.CreateControl<IMyTerminalControlCheckbox, IMyRemoteControl>(IdPrefix + "Checkbox_EnforceHomeOrientation");
                c.Title = MyStringId.GetOrCompute("Enforce Home Orientation");
                c.Tooltip = MyStringId.GetOrCompute("Drone will align to home orientation when docking");
                c.OnText = MySpaceTexts.SwitchText_On;
                c.OffText = MySpaceTexts.SwitchText_Off;
                c.Visible = CustomVisibleCondition;
                c.Getter = (b) => GetBlock(b)?.Terminal_EnforceHomeOrientation ?? false;
                c.Setter = (b, v) =>
                {
                    var logic = GetBlock(b);
                    if (logic != null) logic.Terminal_EnforceHomeOrientation = v;
                };
                c.SupportsMultipleBlocks = true;
                MyAPIGateway.TerminalControls.AddControl<IMyRemoteControl>(c);
            }

            // Home Forward Direction
            {
                var c = MyAPIGateway.TerminalControls.CreateControl<IMyTerminalControlCombobox, IMyRemoteControl>(IdPrefix + "Combo_HomeForwardDirection");
                c.Title = MyStringId.GetOrCompute("Home Forward Direction");
                c.Tooltip = MyStringId.GetOrCompute("Direction drone should face when docked");
                c.Visible = CustomVisibleCondition;
                c.Enabled = (b) => GetBlock(b)?.Terminal_EnforceHomeOrientation ?? false;
                c.ComboBoxContent = (list) =>
                {
                    foreach (Base6Directions.Direction dir in Enum.GetValues(typeof(Base6Directions.Direction)))
                    {
                ...
                c.Getter = (b) => GetBlock(b)?.Terminal_HomeForwardDirectionValue ?? 0;
                c.Setter = (b, v) =>
                {
                    var logic = GetBlock(b);
                    if (logic != null) logic.Terminal_HomeForwardDirectionValue = v;
                };
                c.SupportsMultipleBlocks = true;
                MyAPIGateway.TerminalControls.AddControl<IMyRemoteControl>(c);
            }
            ...
            foreach (Scheduler.TaskType taskType in Enum.GetValues(typeof(Scheduler.TaskType)))
            {
            ...
            foreach (Drone.Capabilities capability in Enum.GetValues(typeof(Drone.Capabilities)))
            {
